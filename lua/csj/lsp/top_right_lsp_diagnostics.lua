...
local function print_diagnostics()
  ...
  if vim.tbl_isempty(line_diagnostics) then
    if ns.user_data.diags then vim.api.nvim_buf_clear_namespace(0, TRLD_nm, 0, -1) end
    return
  end

  if ns.user_data.last_line_nr == linenr and ns.user_data.diags then return end
  ...
  for i, diagnostic in ipairs(line_diagnostics) do
    ...
    for j, dline in ipairs(diag_lines) do
      ...
      if win_info.botline <= x - 1 then return end
...
local function hide_diagnostics()
  ...
  if ok_nm == nil then return end
  local TRLD_nm = vim.diagnostic.get_namespace(ok_nm)
  if TRLD_nm.user_data.diags then vim.api.nvim_buf_clear_namespace(0, ok_nm, 0, -1) end
...
vim.api.nvim_create_autocmd({ 'CursorHold', 'CursorHoldI' }, {
  ...
  -- group = lsp_settings,
  callback = function() return print_diagnostics() end,
})

vim.api.nvim_create_autocmd({ 'CursorMoved', 'CursorMovedI' }, {
  desc = 'Clean the lsp messages from the top right of the screen',
  -- group = lsp_settings,
  callback = function() return hide_diagnostics() end,
})
