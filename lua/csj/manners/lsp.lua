...
function M.keymaps(bufnr)
   ...
   vim.keymap.set({ 'v', 'x' }, '<Leader>ca', ':lua vim.lsp.buf.range_code_action()<CR>', { buffer = bufnr, desc = 'Range code actions' })
   ...
   -- Diagnostics
   vim.keymap.set('n', '<Leader>d', vim.diagnostic.setqflist, { buffer = bufnr, desc = 'Show all diagnostics an a quickfix list' })
   ...
   end, { buffer = bufnr, desc = 'Show a description of the word under cursor' })
