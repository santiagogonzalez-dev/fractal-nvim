...
--- # Highlight groups~
---
--- - `IndentScopeSymbol` - symbol showing on every line of scope.
--- - `IndentScopePrefix` - space before symbol. By default made so as to
...
function MiniIndentscope.setup(config)
   config = H.setup_config(config) -- Setup config
   H.apply_config(config) -- Apply config

   -- Module behavior
   vim.api.nvim_create_augroup('indent_scope', {})
   vim.api.nvim_create_autocmd({ 'CursorMoved', 'CursorMovedI' }, {
      group = 'indent_scope',
      callback = function()
         MiniIndentscope.auto_draw { lazy = true }
      end,
   })
   ...
   -- Create highlighting
   vim.api.nvim_set_hl(0, 'IndentScopeSymbol', { link = 'Delimiter' })
   vim.api.nvim_set_hl(0, 'IndentScopePrefix', { fg = 'NONE', bg = 'NONE', nocombine = true })
end
...
H.border_correctors = {
   ...
   ['both'] = function(line, opts)
      local prev_indent, cur_indent, next_indent = H.get_line_indent(line - 1, opts), H.get_line_indent(line, opts), H.get_line_indent(line + 1, opts)
...
function H.scope_has_intersect(scope_1, scope_2)
   ...
   return (body_2.top <= body_1.top and body_1.top <= body_2.bottom) or (body_1.top <= body_2.top and body_2.top <= body_1.bottom)
end
...
function H.indicator_compute(scope)
   ...
   local virt_text = { { MiniIndentscope.config.symbol, 'IndentScopeSymbol' } }
   ...
   if prefix:len() > 0 then
      table.insert(virt_text, 1, { prefix, 'IndentScopePrefix' })
   end
