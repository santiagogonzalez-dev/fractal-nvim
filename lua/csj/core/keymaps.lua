...
vim.keymap.set('n', '<Leader>p', [["_diwP]], {
  desc = 'Paste in word under the cursor without overwriting the yank register',
})
...
vim.keymap.set(
  ...
  { desc = 'Open NetRW in the directory of the current buffer' }
)
...
vim.keymap.set(
  ...
  '<A-j>',
  ":m '>+1<CR>gv=gv",
  { silent = true, desc = 'Move current block of text up and down' }
) -- Visual mode
vim.keymap.set(
  { 'v', 'x' },
  '<A-k>',
