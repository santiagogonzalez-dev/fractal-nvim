...
if not utils.readable(settings) then
   notify_send('CSJNeovim is not able to locate `settings.json`.')
else
   local ok, USER = pcall(utils.get_json, settings) -- Get user settings.
   if not ok then
      notify_send('CSJNeovim is not able to read `settings.json` properly.')
   else
...
if not utils.readable(user_init) then
   notify_send('CSJNeovim is not able to find an `init.lua` for user.')
else
